...
void reading(const vector<int>& s_data){
...
void writing(const vector<int>& t, vector<int>& s_data){
...
int main(){
    ...
    thread reader1(reading, ref(s_data));
    thread writer1(writing, ref(insert_data), ref(s_data));
    thread reader2(reading, ref(s_data));
    reader1.join();
    reader2.join();
    writer1.join();
    thread reader3(reading, ref(s_data));
    reader3.join();
    auto end = chrono::steady_clock::now();
    auto duration = chrono::duration_cast<chrono::microseconds>(end - start);
